...
inline dual_number relu(const dual_number& x) {
    return x.value() > 0 ? dual_number(x.value(), x.dual()) : dual_number(0.0f, 0.0f);
}
...
public:
    ...
    dual_number& operator[](size_t index) { return data_[index]; }
    const dual_number& operator[](size_t index) const { return data_[index]; }
...
// Element-wise application of functions to vectors
